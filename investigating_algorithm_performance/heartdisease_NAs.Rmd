...
heart_NA_20 <- data.frame(lapply(heart_NA_20, function(x) if(is.character(x)) as.factor(x) else x))
...
# 1. Imputation
imp.data <- mice (data = heart_NA_20, m = 20, maxit = 10, seed = 123, print=FALSE)
...
# You can use the 'complete' function from mice with "long" option to stack all imputed datasets
...
# Now remove the .imp and .id columns to get the final dataset
...
# Assuming 'heart_fit' is your training data and 'final_imputed_data' is your test data
heart.fft <- FFTrees(formula = diagnosis ~ .,
...
# Now you can analyze the FFTrees model
print(heart.fft)
# And you can plot it
plot(heart.fft, data = "test")
