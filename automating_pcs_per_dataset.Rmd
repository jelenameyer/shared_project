...
# Define the vector of pc values:
...
# Create an empty list to store the tables and costs:
tables <- list()
IDK_1_values <- list()

# Loop over each pc value and calculate the table and costs
for (i in 1:length(pcs)) {
  ...
  # New column names for percentages
  ...
  # Calculate the costs using probabilities from the table
  ...
  # Store the IDK_1 value
  IDK_1_values[[as.character(pc)]] <- IDK_1
}

# Access each table using the pc value as the key
tables$`0.75`
IDK_1_values$"0.5"
...
# Calculate IDK_all using probabilities from the table_all
 ...
 # Store the IDK_all value
  IDK_all_values[[as.character(pc)]] <- IDK_all
}

# Access each table and cost using the pc value as the key
tables_all$"0.2"
IDK_all_values$"0.6"
...
for (name in data_names) {
  ...
  # Here, the index `l` is used to iterate over `several_pcs`
  for (l in 1:length(pcs)) {
    input_var <- paste0(name, "_NA_", pcs[l] * 100)
    
    # Row for each pc: in pc
...
# Create an empty list to store the handle_NA_costs values for each pc
handle_NA_costs_list <- list()

# Loop over each pc value
for (l in 1:length(pcs)) {
  
  # Calculate the handle_NA_costs variable for the current pc value
  ...
  # Store the handle_NA_costs value in the list
  handle_NA_costs_list[[as.character(pcs[l])]] <- handle_NA_costs
}

# Access each handle_NA_costs value using the pc value as the key
# For example, to access the handle_NA_costs value for pc = 0.2:
handle_NA_costs_0.2 <- handle_NA_costs_list[["0.2"]]

```

```{r Compare_costs_across_proceedings}
# Define x: ----
x <- seq(-10, 2, length.out = 100)
...
# Initialize an empty vector to store the intersection points
  ...
  for (l in 1:length(pcs)) {
    ...
    # Define the original and FFT_handle functions based on the given inputs:
...
# Initialize an empty vector to store the intersection points
 ...
 # Define the all_IDK function with the specific pc value
...
plot(pcs, all_intersection_points_IDK_1_FFT_handle_titanic , type='o',lty=2,col='darkgreen',lwd=1, main = "Costs of False decisions, where when more expensive 1_IDK starts performing better than FFT_handle.")
abline(h = 0, col='red',lty = 2)

# Add the second line to the plot using the lines function:
...
# plot both and a vertical line at 0, that is the cost of IDK decisions: 
plot(pcs, all_intersection_points_IDK_all_FFT_handle_titanic , type='o',lty=2,ylim = c(-4,2),col='darkgreen',lwd=1,  main = "Costs of False decisions, where when more expensive all_IDK starts performing better than FFT_handle.")
